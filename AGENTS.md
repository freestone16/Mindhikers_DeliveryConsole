# Codex AI - GoldenCrucible-SSE 项目工作流契约
...
- **测试协作口令**：用户在项目目录中说“协调opencode测试”时，默认进入 OpenCode 协同测试模式，先读取：
  - `testing/README.md`
  - `testing/OPENCODE_INIT.md`
  - 当前模块的 `testing/<module>/README.md`
  然后只完成 request / claim / report / status 队列的环境接管；不要再要求用户重复解释协议。完成 ready 后必须等待用户明确说明下一步计划，不能自动发起测试
- **文档位置**：
...
需要浏览器操作时，**严格按以下优先级**使用工具：

| 优先级 | 工具 | 说明 | 何时使用 |
|---|---|---|---|
| **1（最高）** | `agent-browser` skill | 独立无头浏览器，不打扰用户屏幕 | **默认首选**，适用于几乎所有场景 |
| **2** | Linear MCP / 专用 MCP 工具 | 直接 API 调用，无需浏览器 | 有对应 MCP 时优先于浏览器 |
| **3（最低）** | `mcp__Claude_in_Chrome__*` | 控制用户**真实屏幕上的 Chrome**，会打断用户工作流 | **仅在 agent-browser 无法完成时**（如需访问用户已登录的私有页面） |

**规则**：
- ✅ 默认使用 `agent-browser`，独立、不干扰、可并发
- ✅ 有专用 MCP（Linear、GitHub 等）时直接用 MCP，连浏览器都不用开
- ❌ **禁止**在 agent-browser 可以完成任务时使用 `Claude in Chrome`
- ❌ **禁止**未经说明就控制用户的真实浏览器
