...
### 4. First Principles Bug Fixing（第一性原理修 Bug）⭐
- **规则**：**任何 bug 修复，必须先回答"这个设计本身是否正确？"，再动手改代码**
- **禁止行为**：
  - ❌ 只加 `if` 判断覆盖漏掉的字段（补丁思维）
  - ❌ 看到症状立刻改，没有分析设计层级的问题
  - ❌ 修复后逻辑更复杂、更脆弱
- **正确姿势**：
  1. **问根因**：这个 bug 为什么会出现？是设计的逻辑本身有缺陷，还是实现细节的疏漏？
