...
## 2026-03-04 (OpenCode 自定义 LLM Provider 配置)

### 问题：OpenCode 添加自定义 OpenAI 兼容 provider 后，API Key 未传递

**症状**：新开的 OpenCode 窗口找不到自定义模型，或者调用时返回"未提供令牌"错误
...
1. **opencode.json** - 定义 provider 和模型：
```json
{
  "provider": {
    "yinli": {
      ...
      "options": {
        ...
        "apiKey": "sk-xxx"  // ← 必须在 options 里包含 apiKey
      },
      "models": {
        "claude-sonnet-4-6": {
...
**关键点**：
1. 必须指定 `npm: "@ai-sdk/openai-compatible"` 用于 OpenAI 兼容 API
