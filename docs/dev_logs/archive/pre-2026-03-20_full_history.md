...
| v4.3.0-wip | 2026-03-19 | **Director Chatbox联动+上传入口+项目列表** (WIP) — hydration 时序竞态修复、CSET 项目过滤、上传入口无需勾选即显示；预览图崩溃 + chatbox 类型修改不刷新待修 |
...
### 2026-03-19 Director Chatbox 联动 + 上传入口 + 项目列表修复（进行中）

> **状态**：部分已改代码（未 commit），3个核心问题待验证/修复

#### 已完成的代码改动（4个文件，未 commit）

1. **`server/index.ts`**：
   - 新增 `request-expert-hydration` socket handler — 解决 hydration 时序竞态
   - `/api/projects` 过滤条件：`d.name.startsWith('CSET-')` — 只显示 CSET 开头的项目目录
   - `PROJECTS_BASE` 已切换到 `/Users/luzhoua/Mindhikers/Mindhikers_workspace/Projects`

2. **`src/hooks/useExpertState.ts`**：
   - listener 注册后主动 `socket.emit('request-expert-hydration')` 请求后端重发状态
   - 解决页面刷新后 expert state 丢失的问题

3. **`src/components/DirectorSection.tsx`**：
   - 修复初始加载时 `'' → 实际项目ID` 被误判为"切换项目"导致重置 Phase 状态
   - `isInitialSelection` 保护：空字符串 → 实际项目不触发重置

4. **`src/components/director/ChapterCard.tsx`**：
   - 上传入口改为 `requiresUpload` 即显示，不再要求 `isChecked`
   - 删除了旧的"确认后上传"占位提示 — internet-clip/user-capture/artlist 统一直接显示上传区
   - 类型标签改为可点击选中（cursor-pointer 只在标签上）
   - 删除了调试日志 (`ch1-opt1`, `ch2-opt4`)

#### ❌ 待解决问题（下次会话继续）

**问题1：预览图生成导致服务器崩溃**
- **现象**：点击预览图区域触发 `POST /api/director/phase2/thumbnail` → 500 → 服务器进程崩溃
- **复现**：两次复现，均为点击预览图后服务器立即 crash，WebSocket 断连
- **排查方向**：
  - 检查 `server/remotion-api-renderer.ts` 或 `server/director.ts` 中 thumbnail 路由的 uncaught exception
  - 可能是 Remotion CLI 路径问题、文件路径问题、或 props 解析失败
  - 需要捕获具体的 crash stack trace（上次看到的只是 EADDRINUSE 复启失败的日志）
- **关键路由**：`POST /api/director/phase2/thumbnail` — 在 `server/index.ts` 或 `server/director.ts` 中查找

**问题2：Chatbox 修改类型后前端不刷新**
- **现象**：通过 chatbox 发送"2-2 请转换成互联网素材"，系统回复"操作执行成功"，但左侧面板 option 类型标签不更新
- **根因分析进度**：
  - 后端 `update_option_fields` 已改为黑名单模型（IMMUTABLE_FIELDS），type 字段可以正确更新
  - 后端已实现双通道广播：`io.to(room).emit` + `socket.emit` + `chat-action-result` 携带 expertState
  - `useExpertState.ts` 已监听 `chat-action-result` 的 expertState payload
  - **但用户多次确认"还是不行"** — 可能是：
    1. `DirectorSection` 的 `localChapters` state 覆盖了 server broadcast
    2. `chat-action-result` 中没有正确携带 expertState（需检查 `server/index.ts` 的 `chat-action-execute` handler）
    3. 前端 `applyUpdate` 函数的数据解包逻辑不对
- **调试建议**：在 `chat-action-execute` handler 中打日志确认 expertState 是否被正确附加到 result 中

**问题3：上传按钮是否真正生效**
- **已改代码**：ChapterCard 中 `requiresUpload` 不再需要 `isChecked` 即可显示上传区
- **但用户上次报告"artlist实拍等三类视频上传的问题也没解决"** — 需要验证：
  1. 上传区是否渲染出来了（检查 `UPLOAD_REQUIRED_TYPES` 是否包含 `artlist`）
  2. 文件上传 POST 请求是否成功
  3. 上传后 previewUrl 是否正确更新

#### 环境信息

- **PROJECTS_BASE**：`/Users/luzhoua/Mindhikers/Mindhikers_workspace/Projects`（已通过 `.env.local` 配置）
- **项目目录**：CSET-EP4, CSET-EP5, CSET-SP1, CSET-SP2, CSET-SP3, CSET-Seedance2
- **测试项目**：CSET-Seedance2
- **后端端口**：3005（API）/ 5178（Vite proxy）
- **前端端口**：5178
