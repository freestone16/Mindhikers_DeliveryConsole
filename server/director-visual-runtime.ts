...
const PROVIDER_METADATA: Record<GenProvider, { name: string; envVars: string[] }> = {
  volcengine: {
    name: PROVIDER_INFO.volcengine?.name || 'Volcengine (火山引擎)',
    envVars: PROVIDER_INFO.volcengine?.envVars || ['VOLCENGINE_ACCESS_KEY'],
  },
  google: {
...
const getProviderCredential = (provider: GenProvider): string | undefined => {
  const envVar = PROVIDER_METADATA[provider].envVars[0];
...
const assertProviderConfigured = (provider: GenProvider) => {
  ...
  if (!credential) {
    throw new Error(`${PROVIDER_METADATA[provider].name} 未配置凭证，请先在视觉模型配置页保存 API Key。`);
  }
};

const resolveTaskTarget = (taskType: VisualTaskType): ResolvedGenerationTarget => {
  const config = loadConfig();
  const generation = config.generation as Record<string, any>;

  const nextFormat = generation?.[taskType];
  if (
    nextFormat
    && typeof nextFormat === 'object'
    && typeof nextFormat.model === 'string'
    && typeof nextFormat.provider === 'string'
  ) {
    return {
      provider: nextFormat.provider,
      model: nextFormat.model,
    };
  }

  const legacyModel = taskType === 'image' ? generation?.imageModel : generation?.videoModel;
  if (typeof legacyModel === 'string' && legacyModel.trim()) {
    return {
      provider: inferVisualProviderFromModel(taskType, legacyModel) || 'volcengine',
      model: legacyModel,
    };
  }

  const fallbackModel = taskType === 'image'
    ? 'doubao-seedream-5.0-litenew'
...
export const resolveDirectorVisualConfig = (taskType: VisualTaskType): ResolvedDirectorVisualConfig => {
  ...
  return {
    ...
    model: target.model,
    providerLabel: PROVIDER_METADATA[target.provider].name,
