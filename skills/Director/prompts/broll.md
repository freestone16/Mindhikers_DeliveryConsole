...
- **remotion**：适合信息图、认知模型、数据可视化、框架对比等需要图表/**动画**的内容。必须指定 template 和 props（可指定 `theme` 字段）。
  ...
  > - **`overlayOpacity`（遮罩强度）**：当 remotion 方案指定了 `imagePrompt` 底图时，**必须在 `props` 中同时指定 `overlayOpacity`**，控制底图上方暗色遮罩的不透明度（0.0~1.0）。这决定了前景数据与底图之间的视觉平衡。
  >   - 数据展示类模板（ComparisonSplit, NumberCounter, BarChartRace, MetricRings 等）→ **0.7~0.8**（重遮罩，保证数据可读性）
  ...
  > - 自然/科学类 → `microscopic cell structure pattern in dark blue`, `dark topographic contour map texture`, `abstract molecular bond pattern on black`
  > - 商业/经济类 → `dark minimal stock chart grid pattern`, `abstract corporate glass texture with reflections`, `dark blueprint grid with faint technical drawings`
  > - 通用装饰底纹 → `dark carbon fiber weave texture`, `subtle hexagonal mesh on dark gradient`, `dark concrete wall with hairline cracks`, `brushed dark metal texture with subtle grain`
...
5. **imagePrompt 只写英文关键词**，不要写中文句子。格式：`名词 形容词 名词 介词 名词`，例如 `futuristic data center corridor with blue ambient lighting`。
6. **绝对不要在 props 内私自捏造 imageUrl 字段**。imagePrompt 是唯一的图片生成通道。
