...
import type { DirectorCommand, DirectorCommandRequest, Spike } from '../types';
...
export const DirectorControls = ({
  ...
  return (
    <div className="flex flex-col gap-3 text-[var(--gc-text-primary)]">
      {/* 导演指令按钮组 */}
      ...
      {/* 注入观点 */}
      {activeCommand === '投' ? (
        <div className="space-y-2 rounded-lg border border-[var(--gc-line-subtle)] bg-[var(--gc-bg-sunken)] p-3">
          <div className="text-xs text-[var(--gc-text-secondary)]">注入观点</div>
          <Textarea
            className="w-full rounded-lg border border-[var(--gc-line-subtle)] bg-white/60 p-2 text-sm text-[var(--gc-text-primary)] outline-none"
            rows={3}
      ...
      {activeCommand === '深' ? (
        <div className="space-y-2 rounded-lg border border-[var(--gc-line-subtle)] bg-[var(--gc-bg-sunken)] p-3">
          <div className="text-xs text-[var(--gc-text-secondary)]">选择 Spike</div>
          <select
            className="w-full rounded-lg border border-[var(--gc-line-subtle)] bg-white/60 p-2 text-sm text-[var(--gc-text-primary)]"
      ...
      {activeCommand === '换' ? (
        <div className="space-y-2 rounded-lg border border-[var(--gc-line-subtle)] bg-[var(--gc-bg-sunken)] p-3">
          <div className="text-xs text-[var(--gc-text-secondary)]">输入新 persona slug</div>
          <Input
            className="w-full rounded-lg border border-[var(--gc-line-subtle)] bg-white/60 p-2 text-sm text-[var(--gc-text-primary)]"
            type="text"
      ...
      {activeCommand === '？' ? (
        <div className="space-y-2 rounded-lg border border-[var(--gc-line-subtle)] bg-[var(--gc-bg-sunken)] p-3">
          <div className="text-xs text-[var(--gc-text-secondary)]">选择目标哲人</div>
          <select
            className="w-full rounded-lg border border-[var(--gc-line-subtle)] bg-white/60 p-2 text-sm text-[var(--gc-text-primary)]"
            value={targetPersona}
