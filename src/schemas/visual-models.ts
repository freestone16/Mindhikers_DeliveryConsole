...
export const VISUAL_PROVIDER_LABELS: Record<GenProvider, string> = {
...
export const getVisualProvidersForTask = (taskType: VisualTaskType): GenProvider[] => {
  const grouped = taskType === 'image' ? IMAGE_MODELS_BY_PROVIDER : VIDEO_MODELS_BY_PROVIDER;
  return (Object.keys(grouped) as GenProvider[]).filter((provider) => grouped[provider].length > 0);
};
...
export const isVisualModelSupported = (
  taskType: VisualTaskType,
  provider: GenProvider,
  modelId?: string | null,
): boolean => {
  ...
  const grouped = taskType === 'image' ? IMAGE_MODELS_BY_PROVIDER : VIDEO_MODELS_BY_PROVIDER;
  return grouped[provider].some((item) => item.id === modelId);
};
